...
export function createTool(container: IServiceContainer): Tool {
  return new GetLearningProgressTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class GetLearningProgressTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'GetLearningProgress',
    description: 'Gets learning progress.',
    category: 'Learning',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'GetLearningProgress executed successfully', params }
    };
  }
}
