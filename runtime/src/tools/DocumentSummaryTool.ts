...
export function createTool(container: IServiceContainer): Tool {
  return new DocumentSummaryTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class DocumentSummaryTool implements Tool<any, unknown> {
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'DocumentSummary executed successfully', params }
    };
  }
}
