...
export function createTool(container: IServiceContainer): Tool {
  return new EndSessionTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class EndSessionTool implements Tool<any, unknown> {
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'EndSession executed successfully', params }
    };
  }
}
