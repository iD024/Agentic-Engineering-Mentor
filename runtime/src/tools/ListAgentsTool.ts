...
export function createTool(container: IServiceContainer): Tool {
  return new ListAgentsTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class ListAgentsTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'ListAgents',
    description: 'Lists active agents.',
    category: 'Runtime',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'ListAgents executed successfully', params }
    };
  }
}
