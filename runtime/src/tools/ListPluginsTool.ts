...
export function createTool(container: IServiceContainer): Tool {
  return new ListPluginsTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class ListPluginsTool implements Tool<any, unknown> {
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'ListPlugins executed successfully', params }
    };
  }
}
