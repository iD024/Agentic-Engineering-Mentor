...
export function createTool(container: IServiceContainer): Tool {
  return new DescribeToolTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class DescribeToolTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'DescribeTool',
    description: 'Describes a tool.',
    category: 'Tooling',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'DescribeTool executed successfully', params }
    };
  }
}
