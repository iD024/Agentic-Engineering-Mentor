...
export function createTool(container: IServiceContainer): Tool {
  return new ListMilestonesTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class ListMilestonesTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'ListMilestones',
    description: 'Lists learning milestones.',
    category: 'Learning',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'ListMilestones executed successfully', params }
    };
  }
}
