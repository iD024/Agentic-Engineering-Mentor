...
export function createTool(container: IServiceContainer): Tool {
  return new SessionHistoryTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class SessionHistoryTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'SessionHistory',
    description: 'Gets session history.',
    category: 'Session',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'SessionHistory executed successfully', params }
    };
  }
}
