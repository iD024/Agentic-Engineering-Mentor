...
export function createTool(container: IServiceContainer): Tool {
  return new DiscoverCapabilitiesTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class DiscoverCapabilitiesTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'DiscoverCapabilities',
    description: 'Discovers capabilities.',
    category: 'Tooling',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'DiscoverCapabilities executed successfully', params }
    };
  }
}
