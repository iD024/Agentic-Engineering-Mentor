...
export function createTool(container: IServiceContainer): Tool {
  return new ResumeSessionTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class ResumeSessionTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'ResumeSession',
    description: 'Resumes a session.',
    category: 'Session',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'ResumeSession executed successfully', params }
    };
  }
}
