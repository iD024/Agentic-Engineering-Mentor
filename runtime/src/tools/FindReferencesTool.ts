...
export function createTool(container: IServiceContainer): Tool {
  return new FindReferencesTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class FindReferencesTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'FindReferences',
    description: 'Finds references to a symbol.',
    category: 'Repository',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'FindReferences executed successfully', params }
    };
  }
}
