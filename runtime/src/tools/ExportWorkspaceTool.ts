...
export function createTool(container: IServiceContainer): Tool {
  return new ExportWorkspaceTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class ExportWorkspaceTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'ExportWorkspace',
    description: 'Exports a workspace.',
    category: 'Workspace',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'ExportWorkspace executed successfully', params }
    };
  }
}
