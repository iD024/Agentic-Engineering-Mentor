...
export function createTool(container: IServiceContainer): Tool {
  return new ValidateRepositoryTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class ValidateRepositoryTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'ValidateRepository',
    description: 'Validates the repository.',
    category: 'Validation',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'ValidateRepository executed successfully', params }
    };
  }
}
