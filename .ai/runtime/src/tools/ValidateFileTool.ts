...
export function createTool(container: IServiceContainer): Tool {
  return new ValidateFileTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class ValidateFileTool implements Tool<any, unknown> {
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'ValidateFile executed successfully', params }
    };
  }
}
