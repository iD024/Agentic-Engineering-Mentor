...
export function createTool(container: IServiceContainer): Tool {
  return new StaticAnalysisTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class StaticAnalysisTool implements Tool<any, unknown> {
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'StaticAnalysis executed successfully', params }
    };
  }
}
