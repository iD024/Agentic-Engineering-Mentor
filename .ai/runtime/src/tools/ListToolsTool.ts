...
export function createTool(container: IServiceContainer): Tool {
  return new ListToolsTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class ListToolsTool implements Tool<any, unknown> {
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'ListTools executed successfully', params }
    };
  }
}
