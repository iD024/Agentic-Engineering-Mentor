...
export function createTool(container: IServiceContainer): Tool {
  return new ReviewCodeTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class ReviewCodeTool implements Tool<any, unknown> {
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'ReviewCode executed successfully', params }
    };
  }
}
