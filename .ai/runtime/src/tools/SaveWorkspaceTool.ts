...
export function createTool(container: IServiceContainer): Tool {
  return new SaveWorkspaceTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class SaveWorkspaceTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'SaveWorkspace',
    description: 'Saves the current workspace.',
    category: 'Workspace',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'SaveWorkspace executed successfully', params }
    };
  }
}
