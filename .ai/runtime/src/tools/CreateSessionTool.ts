...
export function createTool(container: IServiceContainer): Tool {
  return new CreateSessionTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class CreateSessionTool implements Tool<any, unknown> {
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'CreateSession executed successfully', params }
    };
  }
}
