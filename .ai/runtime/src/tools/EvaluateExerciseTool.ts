...
export function createTool(container: IServiceContainer): Tool {
  return new EvaluateExerciseTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class EvaluateExerciseTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'EvaluateExercise',
    description: 'Evaluates an exercise.',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'EvaluateExercise executed successfully', params }
    };
  }
}
