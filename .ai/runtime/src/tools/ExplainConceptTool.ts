...
export function createTool(container: IServiceContainer): Tool {
  return new ExplainConceptTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class ExplainConceptTool implements Tool<any, unknown> {
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'ExplainConcept executed successfully', params }
    };
  }
}
