...
export function createTool(container: IServiceContainer): Tool {
  return new RuntimeHealthTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class RuntimeHealthTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'RuntimeHealth',
    description: 'Checks runtime health.',
    category: 'Runtime',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'RuntimeHealth executed successfully', params }
    };
  }
}
