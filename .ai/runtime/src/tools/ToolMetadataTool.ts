...
export function createTool(container: IServiceContainer): Tool {
  return new ToolMetadataTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class ToolMetadataTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'ToolMetadata',
    description: 'Gets metadata for a tool.',
    category: 'Tooling',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'ToolMetadata executed successfully', params }
    };
  }
}
