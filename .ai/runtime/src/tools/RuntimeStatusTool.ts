...
export function createTool(container: IServiceContainer): Tool {
  return new RuntimeStatusTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class RuntimeStatusTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'RuntimeStatus',
    description: 'Gets runtime status.',
    category: 'Runtime',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'RuntimeStatus executed successfully', params }
    };
  }
}
