...
export function createTool(container: IServiceContainer): Tool {
  return new GenerateExerciseTool(container.resolve(TOKENS.QueryBus), container.resolve(TOKENS.CommandBus));
}

export class GenerateExerciseTool implements Tool<any, unknown> {
  readonly descriptor: ToolDescriptor = {
    name: 'GenerateExercise',
    description: 'Generates an exercise.',
  ...
  async execute(params: any, context: ToolContext): Promise<ToolResult<unknown>> {
    return {
      success: true,
      data: { message: 'GenerateExercise executed successfully', params }
    };
  }
}
